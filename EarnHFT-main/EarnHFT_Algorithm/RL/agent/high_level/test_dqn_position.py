...
from env.high_level_env import high_level_testing_env, build_model_path_list_dict, load_minute_features, load_second_features
import copy
...
parser.add_argument(
    "--test_path",
    type=str,
    default="result_risk/NVDA/high_level/seed_12345/epoch_1",
...
parser.add_argument(
    "--valid_data_path",
    type=str,
    default="data/NVDA/valid.feather",
    help="the number of hidden_nodes",
)
...
parser.add_argument(
    "--test_data_path",
    type=str,
    default="data/NVDA/test.feather",
    help="the number of hidden_nodes",
)
...
class trader(object):
    def __init__(self, args) -> None:
        ...
        # trading setting
        model_path_list_dict = build_model_path_list_dict(args.dataset_name)
        ...
        self.high_level_tech_indicator_list = load_minute_features(
            args.dataset_name)
        self.low_level_tech_indicator_list = load_second_features(
            args.dataset_name)
    ...
    def act_test(self, info):
        x = torch.tensor(info["high_level_state"]).to(
            self.device).to(torch.float32)
        position = torch.tensor([info["previous_action"]]).to(
            self.device).to(torch.float32).unsqueeze(1)
    ...
    def test(self):
        for label, df in zip(["valid", "test"], self.test_df_list):
            ...
            np.save(os.path.join(self.test_path,
                    label, "action.npy"), action_list)
            np.save(os.path.join(self.test_path,
                    label, "reward.npy"), reward_list)
            np.save(
            ...
            np.save(
                os.path.join(self.test_path, label,
