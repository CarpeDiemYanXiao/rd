...
from RL.util.replay_buffer_DQN import Multi_step_ReplayBuffer_multi_info  # noqa: E402
from env.low_level_env import Testing_env, Training_Env  # noqa: E402
from RL.util.graph import get_test_contrast_curve  # noqa: E402
from RL.util.episode_selector import (  # noqa: E402
    start_selector,
    get_transformation_even_risk,
    get_transformation_even_based_boltzmann_risk,
    get_transformation_even_based_sigmoid_risk,
)
from tool.demonstration import make_q_table_reward_fast  # noqa: E402
...
parser.add_argument("--update_times", type=int,
                    default=1, help="the update times")
...
class DQN(object):
    def __init__(self, args):
        ...
        self.epsilon_decay = (self.epsilon_init -
                              self.epsilon_min) / self.epsilon_step
        ...
        self.tech_indicator_list = np.load(
            f"data/feature/{args.dataset_name}/second_feature.npy").tolist()
    ...
    def update(
        ...
        torch.nn.utils.clip_grad_norm_(
            self.eval_net.parameters(), self.grad_clip)
        self.optimizer.step()
    ...
    def act(self, state, info, epsilon):
        ...
        if np.random.uniform() > epsilon:
            actions_value = self.eval_net.forward(
    ...
    def train(self):
        ...
        epoch_number = 4
        random_position_list = random.choices(
            range(self.action_dim), k=self.num_sample)
        return_rate_list = []
        ...
        for i in range(df_number):
            ...
            idx = min(self.chunk_num, len(df) - 1)
            return_rate_list.append(
                (df.iloc[idx]["bid1_price"] /
                 df.iloc[0]["ask1_price"] - 1)
            )
        ...
        self.start_selector = start_selector(
            range(df_number), initial_priority_list)
        ...
        for sample in range(self.num_sample):
            ...
            self.train_df = pd.read_feather(
                os.path.join(self.train_data_path,
                             "df_{}.feather".format(df_index))
            )
            early_end = max(0, len(self.train_df)-self.chunk_num)

            # ---- Q-table: compute once with disk cache, reuse for both phases ----
            cache_dir = os.path.join(self.train_data_path, ".qtable_cache")
            os.makedirs(cache_dir, exist_ok=True)
            cache_key = "{}_{}_{}_{}_{}_{}".format(
                df_index, self.action_dim, self.max_holding_number,
                self.reward_scale, self.gamma, self.transcation_cost)
            cache_hash = hashlib.md5(cache_key.encode()).hexdigest()[:16]
            cache_file = os.path.join(cache_dir, "{}.npy".format(cache_hash))
            q_table_cached = None
            if os.path.exists(cache_file):
                try:
                    q_table_cached = np.load(cache_file)
                    print("  Q-table loaded from cache for df_{}".format(df_index))
                except Exception:
                    pass
            if q_table_cached is None:
                q_table_cached = make_q_table_reward_fast(
                    self.train_df, num_action=self.action_dim,
                    max_holding=self.max_holding_number,
                    reward_scale=self.reward_scale, gamma=self.gamma,
                    commission_fee=self.transcation_cost)
                try:
                    np.save(cache_file, q_table_cached)
                    print("  Q-table computed & cached for df_{}".format(df_index))
                except Exception:
                    pass
