...
action_dim = 5
# pretty much the same, except the shape of the state, previously, the state is reshape(-1), whose shape is only (N),standing for the number
# of features. Now, the shape is (T,N),standing for num of seconds and number of features. The length of the window is set to 20 to
# correponds to the parameter of the original paper


class env_cqn(gym.Env):
    def __init__(
        self,
        df: pd.DataFrame,
        tech_indicator_list=None,
    ...
    def calculate_avaliable_action(self, price_information):
        ...
        buy_size_max = np.sum(
            price_information[["ask1_size",
                               "ask2_size", "ask3_size", "ask4_size"]]
        )
        sell_size_max = np.sum(
            price_information[["bid1_size",
                               "bid2_size", "bid3_size", "bid4_size"]]
        )
    ...
    def step(self, action):
        ...
        if self.terminal:
            ...
            print(
                "the portfit margine is ",
                self.final_balance / (self.required_money + 1e-12),
            )
        self.reward = self.reward_scale * self.reward
...
if __name__ == "__main__":
    ...
    env = env_cqn(df=df, initial_action=0, dataset_name="NVDA")
